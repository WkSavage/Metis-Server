				...
				if(OFFICER)
					...
					switch(backbag)
						...
						if(4)
							clothes_s.Blend(new /icon('icons/mob/back.dmi', "satchel"), ICON_OVERLAY)
				if(PRISONER)
					clothes_s = new /icon('icons/mob/uniform.dmi', "orange_s")
					clothes_s.Blend(new /icon('icons/mob/feet.dmi', "orange1"), ICON_UNDERLAY)
					if(prob(1))
						clothes_s.Blend(new /icon('icons/mob/head.dmi', "fishbowl"), ICON_OVERLAY)
					switch(backbag)
						if(2)
							clothes_s.Blend(new /icon('icons/mob/back.dmi', "backpack"), ICON_OVERLAY)
						if(3)
